...
trait SmFileCardTable {
  ...
  class SmFileCard(_tableTag: Tag) extends profile.api.Table[SmFileCardRow](_tableTag, "sm_file_card") {
    ...
    /** Index over (deviceUid,sha256,fName) (database name idx_fc_dev_sha_name) */
    val index2 = index("idx_fc_dev_sha_name", (deviceUid, sha256, fName))
    /** Index over (fNameLc) (database name idx_fc_f_name_lc) */
    val index3 = index("idx_fc_f_name_lc", fNameLc)
    /** Index over (fParent,fName) (database name idx_fc_parent_name) */
    val index4 = index("idx_fc_parent_name", (fParent, fName))
    /** Index over (sha256,fName) (database name idx_fc_sha_name) */
    val index5 = index("idx_fc_sha_name", (sha256, fName))
    /** Index over (fLastModifiedDate) (database name idx_last_modified) */
    val index6 = index("idx_last_modified", fLastModifiedDate)
    /** Index over (sha256) (database name idx_sha256) */
    val index7 = index("idx_sha256", sha256)
    /** Index over (deviceUid,fParent) (database name idx_sm_file_card_device_uid) */
    val index8 = index("idx_sm_file_card_device_uid", (deviceUid, fParent))
  }
