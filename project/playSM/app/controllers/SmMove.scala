...
/**
  ...
  */
@Singleton
class SmMove @Inject()(val database: DBService, config: Configuration)
  ...
  def moveAllDevices: Action[AnyContent] = Action.async {
    val maxJob: Int = config.get[Int]("Move.maxJob")
    val maxMoveFiles: Int = config.get[Int]("Move.maxMoveFiles")
