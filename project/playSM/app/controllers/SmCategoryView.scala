...
@Singleton
class SmCategoryView @Inject()(cc: MessagesControllerComponents, config: Configuration, val database: DBService)
  ...
  def listDirWithoutCatByLastDate: Action[AnyContent] = Action.async {
    val maxFilesTake: Long = config.underlying.getBytes("Category.maxFilesTake")
